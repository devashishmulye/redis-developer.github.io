...
Lenders can feed alternative data sourced from a borrower's device using [DeviceConnect](/products/device-connect), tansactions, cashflows  and other proxies such as their network of contacts or social media activity.
...
With FinBox [CollectX](/products/collect-x) we make it easier to collect payments and enforce protocols that preempt defaults. Our algorithms help lenders analyze behavioral patterns, account activity and credit utilization trends to assess if a borrower is likely to default on an upcoming payment. Lenders can then realign their collection resources accordingly, tailoring messages to bororowers based on risk levels, sending reminders on preferred channels and forestalling defaults.
